...
multyears <- combineyears(c(2021, 2022, 2023))
```

```{r}
# function for logistic regression model
vballmodel <- function(data) {
  data <- data %>%
    mutate(`W/L` = as.factor(`W/L`)) %>% 
    mutate(`W/L` = relevel(`W/L`, ref = "L")) # reference is the 0
  ...
  # NOT using SR Rating, we only have that data for 2024
  variable_recipe <- recipe(`W/L` ~ PCT + SA + SE, data = data)
